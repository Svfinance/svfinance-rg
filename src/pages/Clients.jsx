...
export default function Clients() {
  ...
  const [codigoConflito, setCodigoConflito] = useState(null); // { codigo_atual, codigo_novo, payload, url, method }
  ...
  async function openCreate() {
    setEditing(null);
    const proximoCodigo = navigator.onLine ? await buscarProximoCodigo() : "";
    setForm({ ...EMPTY_FORM, codigo: proximoCodigo, emails: [""], phones: [""] });
    setGeoStatus(null);
    setModalOpen(true);
  }
  ...
  function closeModal() {
    ...
    setGeoStatus(null); setCriarCartaoApos(false);
  }
  ...
  async function confirmarSubstituicaoCodigo(substituir) {
    ...
    if (!substituir) return; // admin cancelou — mantém código atual sem alterar
                  ...
                  <tr key={c.id} className="cl-row" style={{ borderBottom:`1px solid ${isGlass?"rgba(255,255,255,0.15)":theme.border}`, transition:"background 0.15s", opacity:c.__pending?0.85:1 }}>
                    ...
                    <td style={{ padding:"12px 16px", verticalAlign:"middle" }}>
                      <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                        ...
                        >!</button>
                        {/* Botão novo cartão — exclusivo RG */}
                        {rg && (
                          <button
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"1fr 1fr", gap:16, marginBottom:8 }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                  ...
                  {geoStatus==="warn" && <span style={{ fontSize:11, color:"#f59e0b" }}>⚠️ CEP encontrado mas sem GPS</span>}
                </div>
                <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                  {labelInput("Número")}
                ...
                {/* Início e fim com calendário nativo */}
                <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                ...
                {/* Dias de atendimento — aparece quando tem recorrência */}

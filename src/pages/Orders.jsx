...
function RestauraGlassCardForm({ clients, onSubmit, onCancel, isMobile }) {
  ...
  // Pré-selecionar cliente vindo do Clients.jsx via ?client_id=X
  useEffect(() => {
  ...
  function selecionarCliente(c) {
    setClienteSel(c); setBusca(c.name); setShowDD(false);
  ...
  function abrirLocalizacao() {
    ...
    if (/iPad|iPhone|iPod/.test(navigator.userAgent)) window.open(`maps://maps.apple.com/?q=${q}`, "_blank");
  ...
  const inp = {
    border: `1px solid ${verdeBd}`, borderRadius: 6, padding: "7px 10px",
    background: "#fff", color: "#1a1a1a", fontFamily: "inherit",
    fontSize: "0.85rem", outline: "none", width: "100%", boxSizing: "border-box", colorScheme: "light",
  };
  const section = {
    background: "rgba(255,255,255,0.88)", border: `1px solid ${verdeBd}`,
    borderRadius: 12, padding: isMobile ? "14px" : "18px",
    marginBottom: 14, boxShadow: "0 2px 12px rgba(26,138,60,0.06)",
  };
  ...
  // ── BLOCO CLIENTE (compartilhado pelos dois modos) ──
  const blocoCliente = (
    <div style={section}>
  ...
  // ══════════════════════════════════════════════
  // MODO FÍSICO — layout cartão impresso
  // ══════════════════════════════════════════════
  if (modoForm === "fisico") {
  ...
  // ══════════════════════════════════════════════
  // MODO DIGITAL — layout glassmorphism
  // ══════════════════════════════════════════════
  return (
    <div style={{ fontFamily: "'Segoe UI',Arial,sans-serif", color: "#1a1a1a" }}>
      {toggle}
...
export default function Orders() {
  ...
  useEffect(() => {
    ...
    if (!cid || !isNew) return;
    // Aguarda os clientes carregarem e abre o form
    const tryOpen = setInterval(() => {
      setClients(prev => {
        if (prev.length > 0) {
          ...
          setView("form");
          // Guardar o client_id para pré-selecionar no form
  ...
  return (
    <PageLayout>
      ...
      {checkinOrder&&<CheckinModal order={{...checkinOrder,client_address:checkinOrder.client_address||""}} isGlass={isGlass} isMobile={isMobile} theme={theme} onClose={()=>setCheckinOrder(null)} onSuccess={()=>{cacheInvalidate("sv_orders");loadOverlays();fetchOrders();showToast("Registro salvo!");setTimeout(()=>setCheckinOrder(null),1500);}}/>}

      {checkinSemana&&<CheckinModal order={{...checkinSemana.order,client_address:checkinSemana.order.client_address||""}} isGlass={isGlass} isMobile={isMobile} theme={theme} onClose={()=>setCheckinSemana(null)} onSuccess={(result)=>{cacheInvalidate("sv_orders");loadOverlays();fetchOrders();showToast(result?.action==="start"?"Check-in registrado!":"Serviço concluído!");setCheckinSemana(null);}}/>}

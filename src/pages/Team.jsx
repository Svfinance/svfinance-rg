...
export default function Team() {
  ...
  const [filterRole, setFilterRole]       = useState("all");     // ← FILTRO ROLE
  ...
  async function fetchUsers() {
    setLoading(true);
    try {
      const res = await fetch(`${API}/company/users`, { headers: { Authorization: `Bearer ${token()}` } });
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!editing && !form.name.trim())  { showToast("Nome é obrigatório.", "error"); return; }
    if (!editing && !form.email.trim()) { showToast("Email é obrigatório.", "error"); return; }
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) { showToast(editing ? "Usuário atualizado!" : "Usuário criado!"); closeModal(); fetchUsers(); }
  ...
  async function handleToggle(u) {
    try {
      await fetch(`${API}/company/users/${u.id}`, { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token()}` }, body: JSON.stringify({ active: !u.active }) });
      fetchUsers();
  ...
  async function handleDelete(id) {
    try {
      const res  = await fetch(`${API}/company/users/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token()}` } });
      const data = await res.json();
  ...
  const inputDisabled = { ...inputStyle, opacity: 0.5, cursor: "not-allowed" };
      ...
      <div style={{ flex:1, padding: isMobile?"72px 16px 40px":"32px 36px", overflowY:"auto", position:"relative", zIndex:1 }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:28, flexWrap:"wrap", gap:12 }}>
          ...
          <button
            ...
            onClick={() => { if (!isPro && users.length >= 10) { showToast("Limite atingido.", "error"); return; } openCreate(); }}
          >
            + Novo Usuário
          </button>
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns: isMobile?"1fr 1fr":"repeat(3,1fr)", gap:16, marginBottom:28 }}>
          {[
            { icon:"👥", label:"Total",    value: users.length,  color:theme.primary, border: isGlass?"rgba(255,255,255,0.5)":`${theme.primary}44` },
        ...
        <div style={{ display:"flex", gap:12, flexWrap:"wrap", marginBottom:20, alignItems:"center" }}>
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            {["all","admin","financial","seller","stock","viewer"].map(r => (
              <button key={r} style={filterBtn(filterRole===r)} onClick={() => setFilterRole(r)}>
                {r === "all" ? "Todos" : (ROLES.find(x => x.value===r)?.label || r)}
              </button>
            ))}
          </div>
        ...
        <div className="table3d-tm">
          {loading ? (
            ...
            <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"60px 0", gap:12, color:theme.textMuted }}>
              ...
              <p>{search || filterRole !== "all" ? "Nenhum usuário encontrado" : "Nenhum usuário cadastrado"}</p>
            </div>
          ) : (
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"0.88rem", minWidth: isMobile?"480px":"unset" }}>
              ...
              <tbody>
                {filtered.map(u => {
                  ...
                  const nameMatch   = search && (u.name||"").toLowerCase().includes(search.toLowerCase());
                  return (
                    <tr key={u.id} className="tm-row" style={{ borderBottom:`1px solid ${isGlass?"rgba(255,255,255,0.15)":theme.border}` }}>
                      <td style={{ padding:"12px 16px", verticalAlign:"middle" }}>
                        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                          ...
                          <div>
                            <div style={{ fontWeight:600, color: nameMatch ? theme.primary : theme.textPrimary }}>{u.name||"—"}</div>
                      ...
                      {!isMobile && <td style={{ padding:"12px 16px", verticalAlign:"middle", color:theme.textMuted, fontSize:"0.8rem" }}>{ri.desc}</td>}
                      <td style={{ padding:"12px 16px", verticalAlign:"middle" }}>
                        <button style={{ display:"inline-block", padding:"3px 10px", borderRadius:20, fontSize:"0.75rem", fontWeight:600, cursor:"pointer", border:`1px solid ${u.active?theme.income:theme.textMuted}44`, background: u.active?`${theme.income}22`:"rgba(100,116,139,0.12)", color: u.active?theme.income:theme.textMuted }} onClick={() => handleToggle(u)}>
        ...
        {(search || filterRole !== "all") && filtered.length > 0 && (
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display:"flex", flexDirection:"column", gap:16, marginBottom:24 }}>
                <div style={fieldStyle}>
                  ...
                  <input style={inputStyle} required placeholder="Ex: João Silva" value={form.name} onChange={e=>setForm({...form,name:e.target.value})} />
                </div>
                <div style={fieldStyle}>
                  <label style={labelStyle}>Email {editing?"(não editável)":"*"}</label>
                  <input style={editing?inputDisabled:inputStyle} required={!editing} type="email" placeholder="email@empresa.com" value={form.email} disabled={!!editing} onChange={e=>setForm({...form,email:e.target.value})} />
                  {editing && <span style={{ fontSize:"0.75rem", color:theme.textMuted, paddingLeft:4 }}>O email de acesso não pode ser alterado.</span>}
                </div>
                <div style={fieldStyle}>
                  <label style={labelStyle}>{editing?"Nova Senha (deixe em branco para manter)":"Senha *"}</label>
                  <input style={inputStyle} type="password" placeholder="••••••••" required={!editing} value={form.password} onChange={e=>setForm({...form,password:e.target.value})} />
                </div>
                <div style={fieldStyle}>
      ...
      {deleteConfirm && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:999, backdropFilter:"blur(4px)" }} onClick={() => setDeleteConfirm(null)}>
          <div style={{ ...modalBg, border:"1px solid rgba(239,68,68,0.3)", borderRadius:18, padding: isMobile?"24px 20px":32, width: isMobile?"92%":"100%", maxWidth:400, boxShadow: isGlass?"0 20px 60px rgba(0,0,0,0.15)":"0 25px 60px rgba(0,0,0,0.6)" }} onClick={e=>e.stopPropagation()}>
            ...
            <p style={{ color:theme.textSecondary, marginBottom:24 }}>Desativar <strong style={{ color:theme.textPrimary }}>"{deleteConfirm.name}"</strong>? O usuário perderá acesso ao sistema.</p>

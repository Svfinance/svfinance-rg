...
export default function Settings() {
  ...
  const [fiscal, setFiscal]       = useState({
    ...
    bairro:"", municipio:"", uf:"", codigo_municipio:"",
    telefone:"", token_focusnfe:"",
  });
        ...
        .then(r => r.json()).then(d => {
          if (d.id) {
            ...
            setFiscal({
              ...
              telefone:            d.telefone            || "",
              token_focusnfe:      d.token_focusnfe      || "",
            });
          }
        }).catch(() => {});
    }
  ...
  async function handleSaveFiscal(e) {
    ...
    try {
      const res = await fetch(`${API}/company`, {
        method:"PUT",
        headers:{ "Content-Type":"application/json", Authorization:`Bearer ${token()}` },
        body: JSON.stringify({ ...company, ...fiscal }),
      });
      const data = await res.json();
      if (res.ok) showToast("Dados fiscais salvos com sucesso!");
      else showToast(data.msg || "Erro ao salvar.", "error");
    } catch { showToast("Erro de conexão.", "error"); }
  ...
  function handleChangeTheme(id) {
    changeTheme(id); setThemeSaved(true);
  ...
  const card = {
    ...
    borderRadius:   20, padding: isMobile ? "24px 20px" : "32px", marginBottom: 28,
      ...
      <div style={{ flex:1, padding:isMobile?"72px 16px 40px":"32px 40px", overflowY:"auto", position:"relative", zIndex:1 }}>
            ...
            <form onSubmit={handleSaveCompany}>
              ...
              <div style={{ marginBottom:28 }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:20, flexWrap:"wrap" }}>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                    ...
                    <button type="button" onClick={() => logoRef.current?.click()} style={{ ...btnPrimary, padding:"9px 20px", fontSize:"0.85rem" }}>
                      📁 Selecionar Imagem
                    </button>
                    {company.company_logo && (
              ...
              <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"1fr 1fr", gap:18, marginBottom:28 }}>
                <div style={{ ...field, gridColumn:"1 / -1" }}>
                  ...
                  <input className="settings-inp" style={inp} required placeholder="Ex: Minha Empresa Ltda"
                    value={company.company_name} onChange={e => setCompany(c => ({ ...c, company_name:e.target.value }))}/>
                </div>
                <div style={field}>
                  <label style={lbl}>CNPJ</label>
                  <input className="settings-inp" style={inp} placeholder="00.000.000/0000-00"
                    value={company.company_cnpj || ""} onChange={e => setCompany(c => ({ ...c, company_cnpj:e.target.value }))}/>
                </div>
                <div style={field}>
                ...
                <div style={{ ...field, gridColumn:"1 / -1" }}>
                  ...
                  <input className="settings-inp" style={inp} placeholder="Rua, número, cidade, estado"
        ...
        {activeSection === "fiscal" && isAdmin && (
          <form onSubmit={handleSaveFiscal}>
            ...
            <div style={{ background:"rgba(124,58,237,0.08)", border:"1px solid rgba(124,58,237,0.2)", borderRadius:14, padding:"14px 18px", marginBottom:24, display:"flex", gap:12, alignItems:"flex-start" }}>
              ...
              <div>
                ...
                <div style={{ fontSize:"0.82rem", color:theme.textMuted, lineHeight:1.6 }}>
                  Preencha os dados fiscais da sua empresa para habilitar a emissão de notas fiscais eletrônicas.
                  O token do Focus NF-e é fornecido em <strong>focusnfe.com.br → Painel API → Tokens</strong>.
                </div>
              </div>
            </div>
            ...
            <div style={card}>
              <h2 style={{ fontSize:"1rem", fontWeight:700, margin:"0 0 6px", color:theme.textPrimary }}>🏛️ Identificação Fiscal</h2>
              <p style={{ color:theme.textMuted, fontSize:"0.82rem", margin:"0 0 24px" }}>CNPJ, inscrições e regime tributário.</p>
              <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"1fr 1fr", gap:18 }}>
                <div style={field}>
                  <label style={lbl}>CNPJ da Empresa *</label>
                  <input className="settings-inp" style={inp} placeholder="00.000.000/0000-00"
                    value={fiscal.cnpj} onChange={e => setFiscal(f => ({ ...f, cnpj:e.target.value }))}/>
                </div>
                <div style={field}>
                  <label style={lbl}>Regime Tributário *</label>
                  <select className="settings-inp" style={sel}
                    value={fiscal.regime_tributario} onChange={e => setFiscal(f => ({ ...f, regime_tributario:e.target.value }))}>
                    {REGIME_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                  </select>
                </div>
                <div style={field}>
                  <label style={lbl}>Inscrição Estadual</label>
                  <input className="settings-inp" style={inp} placeholder="Somente números ou ISENTO"
                    value={fiscal.inscricao_estadual} onChange={e => setFiscal(f => ({ ...f, inscricao_estadual:e.target.value }))}/>
                </div>
                <div style={field}>
                  <label style={lbl}>Inscrição Municipal</label>
                  <input className="settings-inp" style={inp} placeholder="Para NFS-e (nota de serviço)"
                    value={fiscal.inscricao_municipal} onChange={e => setFiscal(f => ({ ...f, inscricao_municipal:e.target.value }))}/>
                </div>
                <div style={field}>
                  <label style={lbl}>Telefone</label>
                  <input className="settings-inp" style={inp} placeholder="(00) 00000-0000"
                    value={fiscal.telefone} onChange={e => setFiscal(f => ({ ...f, telefone:e.target.value }))}/>
                </div>
              </div>
            </div>
            ...
            {/* Endereço Fiscal */}
            <div style={card}>
              <h2 style={{ fontSize:"1rem", fontWeight:700, margin:"0 0 6px", color:theme.textPrimary }}>📍 Endereço Fiscal</h2>
              <p style={{ color:theme.textMuted, fontSize:"0.82rem", margin:"0 0 24px" }}>Endereço completo conforme cadastro na Receita Federal.</p>
              <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"1fr 1fr 1fr", gap:18 }}>

                {/* CEP com busca automática */}
                <div style={{ ...field, gridColumn:isMobile?"1":"1" }}>
                  <label style={lbl}>CEP *</label>
                  <div style={{ display:"flex", gap:8 }}>
                    <input className="settings-inp" style={{ ...inp, flex:1 }} placeholder="00000-000"
                      value={fiscal.cep} onChange={e => setFiscal(f => ({ ...f, cep:e.target.value }))}
                      onBlur={handleBuscarCep}/>
                    <button type="button" onClick={handleBuscarCep} disabled={loadingCep}
                      style={{ background:theme.primaryGrad, color:"#fff", border:"none", borderRadius:10, padding:"10px 14px", fontWeight:700, cursor:loadingCep?"not-allowed":"pointer", fontSize:"0.82rem", flexShrink:0, opacity:loadingCep?0.7:1 }}>
                      {loadingCep ? "..." : "🔍"}
                    </button>
                  </div>
                  <span style={{ fontSize:"0.7rem", color:theme.textMuted }}>Busca automática pelo ViaCEP</span>
                </div>

                <div style={{ ...field, gridColumn:isMobile?"1":"2 / -1" }}>
                  <label style={lbl}>Logradouro (Rua/Avenida)</label>
                  <input className="settings-inp" style={inp} placeholder="Rua das Flores"
                    value={fiscal.logradouro} onChange={e => setFiscal(f => ({ ...f, logradouro:e.target.value }))}/>
                </div>

                <div style={field}>
                  <label style={lbl}>Número</label>
                  <input className="settings-inp" style={inp} placeholder="123"
                    value={fiscal.numero} onChange={e => setFiscal(f => ({ ...f, numero:e.target.value }))}/>
                </div>
                <div style={field}>
                  <label style={lbl}>Complemento</label>
                  <input className="settings-inp" style={inp} placeholder="Sala 2, Apto..."
                    value={fiscal.complemento} onChange={e => setFiscal(f => ({ ...f, complemento:e.target.value }))}/>
                </div>
                <div style={field}>
                  <label style={lbl}>Bairro</label>
                  <input className="settings-inp" style={inp} placeholder="Centro"
                    value={fiscal.bairro} onChange={e => setFiscal(f => ({ ...f, bairro:e.target.value }))}/>
                </div>
                <div style={field}>
                  <label style={lbl}>Município</label>
                  <input className="settings-inp" style={inp} placeholder="Maringá"
                    value={fiscal.municipio} onChange={e => setFiscal(f => ({ ...f, municipio:e.target.value }))}/>
                </div>
                <div style={field}>
                  <label style={lbl}>UF</label>
                  <input className="settings-inp" style={{ ...inp, textTransform:"uppercase" }} placeholder="PR" maxLength={2}
                    value={fiscal.uf} onChange={e => setFiscal(f => ({ ...f, uf:e.target.value.toUpperCase() }))}/>
                </div>
                <div style={field}>
                  <label style={lbl}>Código IBGE do Município</label>
                  <input className="settings-inp" style={inp} placeholder="Ex: 4115200"
                    value={fiscal.codigo_municipio} onChange={e => setFiscal(f => ({ ...f, codigo_municipio:e.target.value }))}/>
                  <span style={{ fontSize:"0.7rem", color:theme.textMuted }}>Preenchido automaticamente via CEP</span>
                </div>
              </div>
            </div>
            ...
            {/* Token Focus NF-e */}
            <div style={card}>
              <h2 style={{ fontSize:"1rem", fontWeight:700, margin:"0 0 6px", color:theme.textPrimary }}>🔑 Token Focus NF-e</h2>
              <p style={{ color:theme.textMuted, fontSize:"0.82rem", margin:"0 0 24px" }}>
                Token de acesso à API do Focus NF-e. Encontrado em <strong style={{ color:theme.textPrimary }}>focusnfe.com.br → Minhas Empresas → Tokens</strong>.
              </p>
              <div style={field}>
                <label style={lbl}>Token de Acesso</label>
                <div style={{ position:"relative" }}>
                  <input className="settings-inp"
                    style={{ ...inp, paddingRight:48, fontFamily:"monospace", fontSize:"0.82rem", letterSpacing:"0.5px" }}
                    type={showToken ? "text" : "password"}
                    placeholder="Cole aqui o token sandbox ou produção"
                    value={fiscal.token_focusnfe}
                    onChange={e => setFiscal(f => ({ ...f, token_focusnfe:e.target.value }))}/>
                ...
                <span style={{ fontSize:"0.72rem", color:theme.textMuted }}>
                  ⚠️ Use o token de <strong>homologação</strong> para testes. Token de produção apenas quando pronto para emitir notas reais.
                </span>
              </div>

              {/* indicador sandbox vs produção */}
              {fiscal.token_focusnfe && (
                <div style={{ marginTop:14, display:"flex", gap:8, alignItems:"center" }}>
                  <div style={{ width:8, height:8, borderRadius:"50%", background: fiscal.token_focusnfe === "66R3bCwxnRBMzyx4QFunliCN8C3GEww1" ? "#f59e0b" : "#22c55e" }}/>
                  <span style={{ fontSize:"0.78rem", color:theme.textMuted }}>
                    {fiscal.token_focusnfe === "66R3bCwxnRBMzyx4QFunliCN8C3GEww1"
                      ? "Token de homologação (sandbox) — notas sem validade fiscal"
                      : "Token personalizado — verifique se é sandbox ou produção"}
                  </span>
                </div>
              )}
            </div>
            ...
            <div style={{ display:"flex", justifyContent:"flex-end", marginBottom:40 }}>
              <button type="submit" disabled={savingFiscal}
                style={{ background:"linear-gradient(135deg,#7c3aed,#6d28d9)", color:"#fff", border:"none", borderRadius:10, padding:"11px 32px", fontWeight:700, cursor:savingFiscal?"not-allowed":"pointer", fontSize:"0.95rem", boxShadow:"0 4px 15px rgba(124,58,237,0.4)", opacity:savingFiscal?0.7:1 }}>
              ...
              <form onSubmit={handleSaveMe}>
                <div style={{ display:"flex", flexDirection:"column", gap:18, marginBottom:28 }}>
                  <div style={field}>
                    ...
                    <input className="settings-inp" style={inp} placeholder="Seu nome"
                  ...
                  <div style={{ borderTop:`1px solid ${isGlass?"rgba(255,255,255,0.2)":theme.border}`, paddingTop:20, marginTop:4 }}>
                    <div style={{ fontWeight:600, color:theme.textPrimary, fontSize:"0.88rem", marginBottom:16 }}>
                      🔑 Alterar Senha <span style={{ fontWeight:400, color:theme.textMuted, fontSize:"0.78rem" }}>(opcional)</span>
                    </div>
                ...
                {[
                  ...
                  return (
                    <div key={s.id} onClick={() => handleSidebarStyle(s.id)}
                      ...
                      {isAct && (
                        <div style={{ position:"absolute", top:10, right:10, background:theme.primaryGrad, color:"#fff", fontSize:10, fontWeight:700, padding:"2px 8px", borderRadius:20 }}>ATIVO</div>
                      )}
                      ...
                      <button onClick={e => { e.stopPropagation(); handleSidebarStyle(s.id); }}
                        style={{ width:"100%", padding:"8px", borderRadius:10, border:"none", cursor:"pointer", fontWeight:600, fontSize:"0.82rem", transition:"all 0.2s",
                          background:isAct?theme.primaryGrad:`${theme.primary}22`, color:isAct?"#fff":theme.primary }}>
                        {isAct ? "✓ Ativo" : "Aplicar"}
                      </button>
                    </div>
                  );
